...
namespace IMAGE_FILTERS
{
    public class ImageOperations
    {
        ...
        {
            for (int j = 0; j < ArrayLength - 1; j++)//O(N2)
            {
                int smallest = j;//o(1)
                for (int i = j + 1; i < ArrayLength; i++)//o(n)
                    if (Array[i] < Array[smallest])//O(n)
                        smallest = i;       //O(1)
                if (smallest != j)
                {
                    byte Temp = Array[j];//o(1)
                    Array[j] = Array[smallest];//O(1)
                    Array[smallest] = Temp;//O(1)
                }
            }
            return Array;
        }
        ...
        //2-QUICK_SORT
        public static int PARTITION(byte[] Array, int p, int r)//o(N log N)
        {
            byte x = Array[r];//O(1)
            byte Temp;
            int i = p;
            for (int j = p; j < r; j++)//O(N LOG N)
            {
                if (Array[j] <= x)//O(LOG N)
                {
                    Temp = Array[j];//O(1)
                    Array[j] = Array[i];//O(1)
                    Array[i++] = Temp;//O(1)
        ...
        //3-COUNTING_SORT
        public static byte[] COUNTING_SORT(byte[] Array, int ArrayLength, byte Max, byte Min)//O(N+k) K is value of Max-Min
        {
